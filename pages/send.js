...
const Send = () => {
    // Checking for window object to avoid errors of using wagmi hooks.
    ...
    return (
        ...
        <div className={`h-full min-h-screen w-full grid grid-cols-[repeat(7,1fr)] grid-rows-[100px,25px,auto,100px] bg-[#131341]`}>
            ...
            <span className={`justify-self-end self-center col-start-1 col-end-8 mx-4 bg-[#372963] flex  `}>

                <ConnectButton />
                <svg className={`cursor-pointer hover:opacity-80 `} width="50" height="50" viewBox="0 0 50 50" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M28.5453 7.7694L38.9773 12.2403C39.345 12.3978 39.5834 12.7594 39.5834 13.1594V30.2456C39.5834 32.1833 38.6476 34.0018 37.0708 35.128L25.5813 43.3348C25.2336 43.5832 24.7665 43.5832 24.4188 43.3348L12.9293 35.128C11.3525 34.0018 10.4167 32.1833 10.4167 30.2456V13.1594C10.4167 12.7594 10.6551 12.3978 11.0228 12.2403L21.4547 7.7694C23.7187 6.79914 26.2814 6.79914 28.5453 7.7694Z" fill="#7E869E" fillOpacity="0.25" />
                    <ellipse cx="25" cy="21.875" rx="1.04167" ry="1.04167" fill="white" />
                    <ellipse cx="19.7917" cy="21.875" rx="1.04167" ry="1.04167" fill="white" />
                    <ellipse cx="30.2084" cy="21.875" rx="1.04167" ry="1.04167" fill="white" />
                </svg>
            ...
            {!isSSR &&
                <React.Fragment>
                    <span className={`rounded-2xl justify-self-center col-start-1 col-end-8 row-start-3 row-end-4 mx-4 p-4  grid grid-rows-[40px,auto,30px,30px,40px,30px,30px,30px,auto,50px] grid-cols-1 border-black border-[2px] bg-[#372963] `}>
                        <span className={`grid grid-col-1 grid-rows-1 p-3`} >
                            <svg className={`justify-self-end hover:opacity-80 cursor-pointer inline `} width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                        ...
                        />
                        <span className={`text-[#20cc9e] text-sm  self-start `}>Select token  </span>
                        ...
                        <span className={`flex `}>
                            {token &&
                                <>
                                    <span className={`text-[#20cc9e] text-sm self-center `}>Balance:  </span>
                                    {token && token?.value == connection.chain?.nativeCurrency.symbol &&
                                        <>
                                            <span className={`text-[#149adc] text-sm self-center ml-2 `}>
                                                {Number(nativeBalance.data?.formatted).toFixed(4)}  {token.value}
                                            </span>
                                            {sendAmount.value != Number(nativeBalance.data?.formatted).toFixed(4) &&
                                                <span className={`text-[#ba7ba2] text-sm self-center ml-4 rounded-xl px-2 bg-[#20cc9e] hover:opacity-90  cursor-pointer `}
                                                    onClick={() => {
                        ...
                        >
                            ...
                            <AccordionDetails>
                                <div className={` border-[1px]  border-[#20cc9e] rounded px-2`}>
                                    <span className={`flex  `}>
                                        <span className={`font-bold text-xs  text-[#372963] self-center`}
                                        >Sending:
                                        </span>
                                        <span className={`flex border-[1px] cursor-pointer border-[#372963] rounded px-2 m-2 `}>
                                            ...
                                            <input
                                                ...
                                                value={addressReciever.substring(0, 4) + "..." + addressReciever.substring(37, 42)}
                                                disabled={true}
                                            />
                                        </span>
                                    </span>
                        ...
                        {!isDisconnected && addressReciever != '' && token && token.value != connection.chain?.nativeCurrency.symbol && Number(tokenBalance.data?.formatted) == 0 &&
                            < button className={`p-2 self-center bg-[#1e1d45] text-[#c24bbe] text-sm opacity-60`} disabled >{`Insufficient ${token.value}`} </button>
                        }
                        {!isDisconnected && addressReciever != '' && token && token.value == connection.chain?.nativeCurrency.symbol && Number(nativeBalance.data?.formatted) == 0 &&
                            < button className={`p-2 self-center bg-[#1e1d45] text-[#c24bbe] text-sm opacity-60`} disabled > {`Insufficient ${token.value}`}</button>
                        }

                        {!isDisconnected && addressReciever != '' && token && token.value != connection.chain?.nativeCurrency.symbol && Number(tokenBalance.data?.formatted) != 0 && Number(sendAmount.value) > Number(tokenBalance.data?.formatted) &&
                            < button className={`p-2 self-center bg-[#1e1d45] text-[#c24bbe] text-sm opacity-60`} disabled >{`Insufficient ${token.value}`} </button>
                        }
                        {!isDisconnected && addressReciever != '' && token && token.value == connection.chain?.nativeCurrency.symbol && Number(nativeBalance.data?.formatted) != 0 && Number(sendAmount.value) > Number(nativeBalance.data?.formatted) &&
                            < button className={`p-2 self-center bg-[#1e1d45] text-[#c24bbe] text-sm opacity-60`} disabled > {`Insufficient ${token.value}`}</button>
                        }
